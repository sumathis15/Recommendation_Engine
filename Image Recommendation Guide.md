...
After a full training run you should have three main artifacts (committed in git). Fashion-MNIST raw files are downloaded locally.

| File | Shape / size | In git? |
|------|----------------|--------|
| `models/fashion_model.pth` | ~43 MB | Yes |
| `features/features.npy` | (60000, 512) | Yes |
| `features/labels.npy` | (60000,) | Yes |
| `data/FashionMNIST/raw/` | 70k images | No — `python scripts/download_fashion_mnist.py` |
...
Index 4271:
  ...
  FashionMNIST train[4271]                      ← the picture (after download)
```
...
The app **never** searches by name. Catalog **indices** (e.g. 4271) are chosen **after** similarity search:

```
Upload pixels → embedding → compare 60k vectors → indices [882, 4271, ...]
                                                      ↓
                              FashionMNIST train[882], train[4271], ... to show
```
...
```
60k TRAIN  →  train model  →  features.npy (catalog)
              download Fashion-MNIST raw for display in app
...
### `data/FashionMNIST/` — dataset (download locally)

| | |
|---|---|
| **Contents** | Official raw idx files — 60k train + 10k test |
...
| **Used after search?** | **Yes** — app shows `train[i]` for catalog index `i` |
| **In git?** | **No** — run `python scripts/download_fashion_mnist.py` |

### `data/test_upload/` — manual test uploads (in git)
...
| Folder | Role | Analogy |
|--------|------|---------|
| `FashionMNIST/` (train split) | Catalog images the app displays | Products on store shelves |
| `test_upload/` | Sample photos to try uploads | Photos you bring in to ask “find similar” |
...
Model embeds upload live
        ↓
Search 60k TRAIN catalog (features.npy)
        ↓
Display Fashion-MNIST train images at top-5 indices
```
...
sequenceDiagram
    ...
    participant FM as FashionMNIST train
    ...
    S->>U: 5 images + class + catalog # + visual match score
```
...
│   └── download_fashion_mnist.py  # Download 70k Fashion-MNIST raw files
│
├── utils/
...
│
├── data/
│   ├── README.md              # FashionMNIST vs test_upload
│   ├── FashionMNIST/          # Downloaded dataset (local only)
│   └── test_upload/           # Demo upload images (in git)
│
├── models/
│   └── fashion_model.pth      # Trained weights (in repo)
│
└── features/
...
flowchart LR
    ...
    APP --> LAB
    APP --> UTILS[utils/]
    APP --> FM[FashionMNIST train]

    DL[download_fashion_mnist.py] --> FM
```
...
### Download Fashion-MNIST (for app display)

```bash
python scripts/download_fashion_mnist.py
```

Downloads 60k train + 10k test raw files to `data/FashionMNIST/raw/`. Catalog index `i` maps to training image `i`.
...
Checks shapes, L2 normalization, model load, test accuracy (~92.81%), and Fashion-MNIST raw files.
